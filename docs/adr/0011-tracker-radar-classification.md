...
把 T41 的识别带上 Web 仪表盘时，因 T41 只有「归属」无类别，**T42 也随之从「类别维度」reframe 成「追踪器暴露」维度**：`/api/trackers` 返回切片内「多少连接命中已知追踪器」+ 按归属公司 top-N，仪表盘出占比条 + 归属排名。归类仍在 Go 侧（DuckDB 无公共后缀函数）：取全量 host 计数（`HostCounts`）→ 归类器归并（`computeTrackerBreakdown`），**不物化维表**、每请求在内存算（此规模够用）。两条已知 caveat：

- **归属不可钻取**：归属公司不是后端 `Filter` 维度（无 owner 列/无法按 owner 过滤），故归属条形有意不做钻取——与 T41 类别缺失同源，非疏漏。
- **serve 常驻需重启才生效新数据**：归类器在 `serve` 启动时加载一次；对长驻守护进程跑 `inhomo tracker update` 后，需重启 serve 才生效（`audit` 每次运行重载，无此问题）。可接受，留作后续优化（懒重载/按 mtime 刷新）。
