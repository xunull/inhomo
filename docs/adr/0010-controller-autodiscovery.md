...
- **机制 = 读配置 + 探活**：读本机已知客户端生成的运行时 mihomo 配置（v0 为 Clash Verge Rev，按 OS 定位其 app-data 下 `config.yaml`）→ 解析出候选 `(controller, secret, 来源)`（`external-controller` 走 TCP、`external-controller-unix` 走 socket，unix 优先）→ 逐个用 `/version` 探活（带上 secret、短超时）→ 用**第一个活的**（200 = 端点活着且 secret 被接受）。
...
裸 mihomo（非默认端口 / 带 secret）配置源（留作 T40）；Windows 上的客户端路径；发现多个活控制器时的交互式选择（固定按来源顺序取第一个）；把发现结果缓存到磁盘。
