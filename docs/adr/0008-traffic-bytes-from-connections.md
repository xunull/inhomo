...
- **流量拓扑按度量加权（T32/T33）**：`/api/flow` 加 `metric`——连接数走 connections（全量）、`up`/`down`/`total` 走 traffic（抽样）。本 ADR「两套数据集、按度量选表」直接适用，拓扑字节口径的短连接漏计取舍与此处一致；即兑现上文「v1 不做」里「度量切换融进现有面板」的那一项（落到流量拓扑）。
